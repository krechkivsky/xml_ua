...
from .common import logFile, log_msg
...
class CadastralParcel:
    ...
    def add_parcel_layer(self):
        ...
        # --- Кінець змін ---
        self.layer.loadNamedStyle(os.path.join(self.plugin_dir, "templates", "parcel.qml"))
        ...
        try:
            ...
            log_msg(logFile, f"Не вдалося створити GeometryProcessor в CadastralParcel: {e}")
            processor = None
        ...
        if parcel_metric_info is not None:
            ...
            if processor and externals_element is not None:
                ...
                log_msg(logFile, f"Додано Ділянку'{object_shape}'") # type: ignore
            ...
            feature.setAttributes([parcel_id, area, object_shape])
